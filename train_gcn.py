...
if __name__ == "__main__":
    ...
    if args.problem == 'setcover':
        train_files = [str(path) for path in Path("data/samples/setcover/train/").glob("sample_*.pkl")]
        valid_files = [str(path) for path in Path("data/samples/setcover/valid/").glob("sample_*.pkl")]
        running_dir = 'trained_gcn_models/setcover'

    elif args.problem == 'cauctions':
        train_files = [str(path) for path in Path("data/samples/cauctions/train/").glob("sample_*.pkl")]
        valid_files = [str(path) for path in Path("data/samples/cauctions/valid/").glob("sample_*.pkl")]
        running_dir = 'trained_gcn_models/cauctions'

    elif args.problem == 'facilities':
        train_files = [str(path) for path in Path("data/samples/facilities/train/").glob("sample_*.pkl")]
        valid_files = [str(path) for path in Path("data/samples/facilities/valid/").glob("sample_*.pkl")]
        running_dir = 'trained_gcn_models/facilities'

    elif args.problem == 'indset':
        train_files = [str(path) for path in Path("data/samples/indset/train/").glob("sample_*.pkl")]
        valid_files = [str(path) for path in Path("data/samples/indset/valid/").glob("sample_*.pkl")]
        running_dir = 'trained_gcn_models/indset'
    ...
    from policies.utilities import log, pad_tensor, Scheduler
    sys.path.insert(0,'.')
    from policies.gcn import GNNPolicy,  GraphDataset
